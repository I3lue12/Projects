...
namespace Adventure
{
    public class Game
    {
        ...
        public Game(int width, int height)
        {
            ...
            this.height = height;
            CreateAnimations();
            CreateForest(3 * width / 4, height / 4, width / 4, height / 2, 70);
            CreateForest(width / 4, 3* height / 4, width / 2, height / 3, 50);
            ...
            Projectile arrow = new Projectile("arrow", new Vector(), animations["arrow"], new Vector(),
                500, false, 5, null);
            Bow bow = new Bow("longbow", new Vector(), null, 5, 30, arrow);
            link.Inventory.Add(bow);
            link.EquippedWeapon = bow;

            Bow bowOfOrion = new Bow("Bow of Orion", new Vector(rand.Next(0, width), rand.Next(0, height)), animations["linkRanged"],
                15, 40, arrow);
            map.Add(bowOfOrion);

            Sword excalibur = new Sword("Excalibur", new Vector(rand.Next(0, width), rand.Next(0, height)), animations["linkNone"],
                25);
            map.Add(excalibur);
        ...
        protected void RemoveDeadGuys()
        {
            if (mobsToDelete.Count > 0)
            {
                ...
                {
                    obituary += monsters[i].Type.ToString() + " has just perished. God rest his soul. ";
                    monsters.RemoveAt(i);
                }
                    ...
                    || (attacker.Type == MobType.Human && m.Type != MobType.Human))
                {
                    ...
                    if (dist <= attacker.EquippedWeapon.MaxRange)
                    {
                        ...
                        {   // Make sure target is IN FRONT of us when we attacked
                            int totDamage = attacker.Attack + attacker.EquippedWeapon.Value;
                            totDamage += rand.Next(-5, 6);
                            totDamage -= m.Defense;// target's defense subtracts from damage
                            if (totDamage < 0)
                            {
                                totDamage = 0;
                            }
                            m.HP -= totDamage;// mob takes damage
                            obituary += attacker.Type.ToString() + " did " +
                                totDamage + " damage to " + m.Type.ToString()
                                + " who has " + m.HP + "HP left";
                            //                        Text = m.Name + " took " + totDamage + " damage and his HP = " + m.HP;
                        }
                    }
                }
            }
        ...
        public void UseItem(Item item)
        {
            if (item is Weapon)
            {
                link.EquippedWeapon = (Weapon)item;
            }
            //            else if (i is Armor)
            else 
            {
        ...
        protected void PlaceRandomPotions(int numPotions)
        {
            for (int i = 0; i < numPotions; i++)
            {
                ...
                Potion p = new Potion(name, new Vector(x, y), animations["potion"], strength, PotionType.Health);
                map.Add(p);
            }
        }
